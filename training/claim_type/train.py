...
LABELS = ["factual", "opinion", "mixed", "numerical"]
...
def load_model_and_tokenizer(model_candidates):
    ...
    for model_name in model_candidates:
        try:
            ...
            last_exc = exc
    raise RuntimeError(f"Unable to load any configured claim-type model: {last_exc}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Train claim type classifier.")
    parser.add_argument("--config", default="training/configs/claim_type.yaml")
    ...
    save_run_metrics(config["output"]["metrics_dir"], "claim_type", metrics, payload)
