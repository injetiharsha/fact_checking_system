...
def load_model_and_tokenizer(model_candidates):
    ...
    for model_name in model_candidates:
        try:
            ...
            last_exc = exc
    raise RuntimeError(f"Unable to load any configured claim-checkability model: {last_exc}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Train claim checkability classifier.")
    parser.add_argument("--config", default="training/configs/claim_checkability.yaml")
    ...
    dataset = load_dataset("json", data_files={
        ...
        **({"test": str(Path(config["data"]["test_file"]))} if config["data"].get("test_file") else {}),
    })
    model_name, tokenizer, model = load_model_and_tokenizer(model_candidates)

    def preprocess(batch):
        tokens = tokenizer(batch["text"], truncation=True, max_length=int(config["training"].get("max_length", 128)))
    ...
    save_run_metrics(config["output"]["metrics_dir"], "claim_checkability", metrics, payload)
