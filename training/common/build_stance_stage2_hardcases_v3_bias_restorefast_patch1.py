...
def normalize_patch_rows(rows):
    ...
    for idx, row in enumerate(rows, start=1):
        normalized.append({
            'id': row.get('id') or f'restorefast_patch1_{idx}',
            'claim': row['claim'],
            'evidence': row['evidence'],
            'label': row['label'],
            'source': row.get('source') or 'restorefast_patch1',
            'source_weight': float(row.get('source_weight', 1.0)),
            'weak_label': bool(row.get('weak_label', False)),
            'residual_type': row.get('residual_type', 'restorefast_patch1'),
            'notes': row.get('notes'),
        })
    return normalized
...
def main():
    parser = argparse.ArgumentParser(description='Build stance v3_bias restorefast patch1 dataset.')
    parser.add_argument('--base-dir', default='data/stance/stage2_hardcases_v3_bias')
    parser.add_argument('--patch-file', default='data/stance/residuals/stance_restorefast_patch1_residual.jsonl')
    parser.add_argument('--output-dir', default='data/stance/stage2_hardcases_v3_bias_restorefast_patch1')
